...
const WeatherDisplay = (props) => {
    ...
    //компонент отображает их, в противном случае делает запрос на api по активному городу
    useEffect(() => {
        const {activeCity, weatherDataByGeo} = props;

        weatherDataByGeo ?
            setState({weatherData: weatherDataByGeo, loading: false}) :
            fetchData(activeCity);
    }, [])
    ...
    useEffect(() => {
        ...
        prevProps.activeCity !== activeCity && fetchData(activeCity);
        prevProps.weatherDataByGeo !== weatherDataByGeo && setState({weatherData: weatherDataByGeo, loading: false});
    }, [])

        const {weatherData, err, loading} = state;
